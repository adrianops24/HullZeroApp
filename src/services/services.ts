...
export const foulingService = {
  ...
  async predictAdvanced(vesselId: string, useAdvanced: boolean = true) {
    try {
      ...
      const response = await apiClient.post(`/api/db/vessels/${vesselId}/fouling/predict?use_advanced=${useAdvanced}`);
      return response.data as AdvancedFoulingPrediction;
    } catch {
      // Fallback para endpoint antigo se necessário
      const response = await apiClient.post(`/api/vessels/${vesselId}/fouling/predict/advanced`, {
        vessel_id: vesselId
      });
      return response.data as AdvancedFoulingPrediction;
    }
  ...
  async predict(vesselId: string) {
    try {
      // Usar endpoint do banco que carrega dados reais automaticamente
      const response = await apiClient.post(`/api/db/vessels/${vesselId}/fouling/predict?use_advanced=false`);
      return response.data;
    } catch {
      // Fallback para endpoint antigo se necessário
      const response = await apiClient.post(`/api/vessels/${vesselId}/fouling/predict`, { vessel_id: vesselId });
      return response.data;
    }
  }
};
...
export const operationalDataService = {
  ...
  async getHistory(vesselId: string, startDate?: string, endDate?: string) {
    ...
    const response = await apiClient.get(`/api/vessels/${vesselId}/operational-data?${params.toString()}`);
    return response.data;
  },
...
export const complianceService = {
  ...
  async predictRisk(vesselId: string, data: any) {
    const response = await apiClient.post(`/api/vessels/${vesselId}/normam401/risk-predict`, data);
  ...
  async getComplianceChecks(vesselId: string, startDate?: string, endDate?: string, limit: number = 100) {
    try {
      ...
      const response = await apiClient.get(`/api/compliance/vessels/${vesselId}/checks?${params.toString()}`);
      return response.data;
    } catch {
  ...
  async getLatestComplianceCheck(vesselId: string) {
    try {
      const response = await apiClient.get(`/api/compliance/vessels/${vesselId}/checks/latest`);
      return response.data;
    } catch {
  ...
  async getInspections(
    ...
    try {
      ...
      const response = await apiClient.get(`/api/compliance/vessels/${vesselId}/inspections?${params.toString()}`);
      return response.data;
    } catch {
      return [];
    }
  ...
  async getComplianceSummary(status?: string) {
    try {
      ...
      const response = await apiClient.get(`/api/compliance/status/summary?${params.toString()}`);
      return response.data;
    } catch {
      return null;
    }
